...
var SHA256 = class extends SHA2 {
  ...
  get() {
    const { A: A3, B: B2, C, D, E: E2, F, G, H } = this;
    return [A3, B2, C, D, E2, F, G, H];
  }
  // prettier-ignore
  set(A3, B2, C, D, E2, F, G, H) {
    this.A = A3 | 0;
    this.B = B2 | 0;
    this.C = C | 0;
    this.D = D | 0;
  ...
  process(view, offset) {
    ...
    let { A: A3, B: B2, C, D, E: E2, F, G, H } = this;
    for (let i3 = 0; i3 < 64; i3++) {
      const sigma1 = rotr(E2, 6) ^ rotr(E2, 11) ^ rotr(E2, 25);
      const T1 = H + sigma1 + Chi(E2, F, G) + SHA256_K[i3] + SHA256_W[i3] | 0;
...
function assertNumber(n9) {
  if (!Number.isSafeInteger(n9))
    throw new Error(`Wrong integer: ${n9}`);
}
...
function convertRadix2(data, from, to, padding2) {
  ...
  const mask = 2 ** to - 1;
  const res = [];
  for (const n9 of data) {
    assertNumber(n9);
    if (n9 >= 2 ** from)
      throw new Error(`convertRadix2: invalid data word=${n9} from=${from}`);
    carry = carry << from | n9;
    if (pos + from > 32)
...
var getMasterKeyFromSeed = (seed) => {
  const h3 = hmac.create(sha512, HMAC_KEY);
  const I = h3.update(hexToBytes(seed)).digest();
...
function serializeVectorWithFunc(value, func) {
  const serializer = new Serializer();
  serializer.serializeU32AsUleb128(value.length);
  const f3 = serializer[func];
  value.forEach((item) => {
    f3.call(serializer, item);
  });
...
var MultiEd25519PublicKey = class _MultiEd25519PublicKey {
  ...
  toBytes() {
    ...
    this.public_keys.forEach((k2, i3) => {
      bytes2.set(k2.value, i3 * Ed25519PublicKey.LENGTH);
    });
  ...
  static deserialize(deserializer) {
    ...
    const keys = [];
    for (let i3 = 0; i3 < bytes2.length - 1; i3 += Ed25519PublicKey.LENGTH) {
      const begin = i3;
...
var _MultiEd25519Signature = class _MultiEd25519Signature2 {
  ...
  toBytes() {
    ...
    this.signatures.forEach((k2, i3) => {
      bytes2.set(k2.value, i3 * Ed25519Signature.LENGTH);
    });
  ...
  static deserialize(deserializer) {
    ...
    const sigs = [];
    for (let i3 = 0; i3 < bytes2.length - bitmap.length; i3 += Ed25519Signature.LENGTH) {
      const begin = i3;
...
function nextToken(tagStr, pos) {
  ...
  if (c === ":") {
    ...
    let res = "";
    for (let i3 = pos; i3 < tagStr.length; i3 += 1) {
      const char = tagStr[i3];
...
var TransactionBuilderABI = class _TransactionBuilderABI {
  ...
  static toBCSArgs(abiArgs, args) {
    ...
    return args.map((arg, i3) => {
      const serializer = new Serializer();
      serializeArg(arg, abiArgs[i3].type_tag, serializer);
      return serializer.getBytes();
    });
  }
  static toTransactionArguments(abiArgs, args) {
    ...
    return args.map((arg, i3) => argToTransactionArgument(arg, abiArgs[i3].type_tag));
  }
...
var TransactionBuilderRemoteABI = class {
  ...
  async build(func, ty_tags, args) {
    ...
    const typeArgABIs = abiArgs.map(
      (abiArg, i3) => new ArgumentABI(`var${i3}`, new TypeTagParser(abiArg, ty_tags).parseTypeTag())
    );
    const entryFunctionABI = new EntryFunctionABI(
      ...
      "",
      // Doc string
      funcAbi.generic_type_params.map((_, i3) => new TypeArgumentABI(`${i3}`)),
      typeArgABIs
    );
...
var _AptosClient = class _AptosClient2 {
  ...
  async simulateTransaction(accountOrPubkey, rawTransaction, query) {
    ...
    if (accountOrPubkey instanceof AptosAccount) {
      ...
      const txnBuilder = new TransactionBuilderMultiEd25519(() => {
        ...
        const bits = [];
        const signatures = [];
        for (let i3 = 0; i3 < threshold; i3 += 1) {
          bits.push(i3);
  ...
  async estimateMaxGasAmount(forAccount) {
    ...
    const accountResource = resources.find((r6) => r6.type === typeTag);
...
var n = ((e10) => (e10[e10.Unauthorized = 4100] = "Unauthorized", e10[e10.InternalError = -30001] = "InternalError", e10))(n || {});
  ...
  constructor(t9 = {}) {
    this.option = t9, this.url = "https://www.twallet.ai", this.version = "1.0.0", this.name = "T wallet", this.icon = "data:image/png;base64,", this.chains = e6, this.accounts = [], this.provider = typeof window < "u" ? window.dekey : void 0, this.initializing = false, this.account = async () => {
      ...
      if (!this.initializing) {
        ...
        const a2 = f2((n9 = this.option) == null ? void 0 : n9.env);
        if (!a2)
          throw this.initializing = false, new Error(`walletDomain not found with env: ${(r6 = this.option) == null ? void 0 : r6.env}`);
      ...
      const e10 = await ((o8 = this.provider) == null ? void 0 : o8.request({
    ...
    }, this.checkProvider = async () => new Promise((s2) => {
      setTimeout(() => {
        s2(false);
      }, 1e4);
      const e10 = setInterval(() => {
        this.provider && (s2(true), clearInterval(e10));
      }, 100);
    }), this.network = async () => {
      const { chainId: s2, name: e10, url: n9 } = await this.provider.request({
    ...
    }, this.disconnect = async () => Promise.resolve(), this.signTransaction = async (s2, e10) => {
      try {
        const n9 = s2.rawTransaction.bcsToBytes(), { concatedSig: r6, uncompressedPubkey: o8 } = await this.provider.request({
          method: "aptos_signTransaction",
          params: [Array.from(n9)]
        }), a2 = new m(o8), l3 = new l2(r6), w = new X(
          new k(a2),
          new O(l3)
        );
    ...
    }, this.signAndSubmitTransaction = async (s2) => {
      const e10 = s2.rawTransaction.bcsToBytes(), n9 = await this.provider.request({
    ...
    }, this.signMessage = async (s2) => {
      var e10;
      try {
        const n9 = await ((e10 = this.provider) == null ? void 0 : e10.request({
    ...
    }, this.onAccountChange = async () => Promise.resolve(), this.onNetworkChange = async (s2) => {
      const e10 = async () => {
        const n9 = await this.network(), { name: r6, chainId: o8, url: a2 } = n9;
        s2({
          name: r6,
          chainId: o8,
          url: a2
    ...
    }, !(t9 != null && t9.env) && (t9 == null ? void 0 : t9.registerWallet) === true && u(this);
  }
  get features() {
    return {
      "aptos:connect": {
...
var f2 = (i3) => i3 === "LOCAL" ? "http://localhost:5173" : i3 === "DEV" ? "https://dev.twallet.ai" : i3 === "QA" ? "https://qa.twallet.ai" : "https://www.twallet.ai";
var u = (i3) => {
  typeof window > "u" || registerWallet(i3);
};
