  ...
  measure(option, threshold) {
    ...
    for (let i = 1; i <= aLength; i++) {
      ...
      for (let j = 1; j <= bLength; j++) {
        ...
        if (i > 1 && j > 1 && a3[i - 1] === b[j - 2] && a3[i - 2] === b[j - 1]) {
...
function mergeValues(a3, b) {
  ...
  if (a3 === b) {
    ...
    for (const key of sharedKeys) {
      const sharedValue = mergeValues(a3[key], b[key]);
  ...
  } else if (aType === ZodParsedType.date && bType === ZodParsedType.date && +a3 === +b) {
    return { valid: true, data: a3 };
  } else {
    return { valid: false };
  }
...
async function generateChunksetCommitments(shouldPad, chunksetIdx, chunksetData, expectedChunksetSize, provider, onChunk) {
  ...
  const a3 = await generateMerkleRoot(
    chunkRoots.map((a22) => a.fromHexInput(a22))
  );
  const entry = {
    chunkset_root: a3.toString(),
...
var ShelbyClient = class {
  ...
  constructor(config, provider) {
    ...
    this.config = config;
    this.aptos = new ee(getAptosConfig(config));
